...
clone_and_copy_modules() {
    ...
    if [[ $repo_type == "enterprise" ]]; then
        ...
        local copy_all_modules=false
        if [[ "${modules_conditions[0]}" = "copyall" ]]; then
        ...
        # Copy all modules if copy_all_modules is true
        if [[ $copy_all_modules == true && $should_clone == true ]]; then
            echo "Copying all modules from ${repo_name} into ${THIRD_PARTY_ADDONS}"
            cp -r "/${repo_name}" "${THIRD_PARTY_ADDONS}/"
        else
